...
def test_ll(self):
    ...
    # click the login button - user must be logged in to view the
...
elem = driver.find_element(By.ID, "id_password")
        ...
        time.sleep(3)  # pause to allow screen to load
        # find 'Customers' and click it – note this is all one Python
statement
        elem = driver.find_element(By.XPATH,
'//*[@id="myNavbar"]/ul[1]/li[3]/a').click()
        time.sleep(3)  # pause to allow screen to change
        try:
            # verify Summary button exists on new screen after clicking
"Customers" button
            elem = driver.find_element(By.XPATH,
                                       '//*[@id="app-
layout"]/div[2]/div/div[3]/table/tbody/tr/td[13]/a')
            print("Test passed - Customer list displayed")
            assert True
        except NoSuchElementException:
            self.fail("Customer List does not appear when Customers is
clicked - test failed")
def tearDown(self):
    self.driver.close()
